...
fun KataList(viewModel: KataViewModel, navController: NavHostController) {
    Scaffold(
        topBar = {
            TopAppBar(title = {
                Row(
                    ...
                    Text("Shotokan Karate Katas", fontSize = TextUnit(25.0F, TextUnitType.Sp), fontWeight = FontWeight.Bold)
                }
            })
        }
    ) { paddingValues ->
        Column(
            ...
            viewModel.getAllKataRecords().map {
                KataListItem(
                    info = KataInfo.findById(it.kataId)!!,
                    viewModel = viewModel,
                    navController
                )
...
fun KataListItem(info: KataInfo, viewModel: KataViewModel, navController: NavHostController) {
    val sessionCount = remember {
        mutableIntStateOf(viewModel.getSessionsForKata(info))
    }
    OutlinedButton(
        onClick = {
        ...
        Row(
            ...
            Column {
                ...
                Text(text = "Sessions: ${sessionCount.intValue}")
            }
            key(sessionCount.intValue) {
                Button(onClick = {
                    viewModel.updateKata(info)
                    sessionCount.intValue = viewModel.getSessionsForKata(info)
                }) {
